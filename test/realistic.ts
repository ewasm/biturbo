...
import { AccountData, accountDataFromJSON, turboproofFromAccountData, toBuffer, generateRealisticTestSuite } from '../src/relayer/realistic'
import { main } from '../src/ee'
const Trie = require('merkle-patricia-tree/secure')

tape('turboproof from rpc data', async t => {
  const testCase = JSON.parse(fs.readFileSync(path.join(__dirname, 'fixture/eth_getProof_sample.json'), { encoding: 'utf8' }))
  const accData = []
